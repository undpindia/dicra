...
# NOTE: Use this method if each filename has the year, month and day at the exact same position
...
# Read data from tiff files and put mean pixel values per polygon within a dataframe
# NOTE: Use this method if the filenames do not have the year, month and day at the exact same position
def create_df2(colname, boundaries, folder_path, method, date_format):
    columns =['index', colname,'geometry']
    param_df = boundaries[columns].copy()
    ...
    for i in file_names:
        ...
        # For each image, we calculate the mean pixel/parameter value for every geometry within the dataframe
        zonal_param = zonal_stats(boundaries, param_array, affine = affine, stats=['mean'], geojson_out = True, nodata = param.nodata, all_touched = method)
        date = re.search('.*\_(.\d+?)\_.*', i).group(1)
...
def dppd_function(data, state, beginyear, endyear, param, boundaries, level_name, period, seasonal):
    ...
    # Save the Trend scores to a geojson file
    ...
    DPPD_df.plot(column = 'Slope Score', 
                        ...
                        cmap = 'RdYlBu_r',   #'RdYlGn'
                        norm = norm)
    plt.title(text)

    # Save the image
    #plt.savefig('C:/Users/mieke/Pictures/DPPD_analysis/Telangana/deviance_analysis_' + param + '_' + level_name + '_' + str(beginyear) + '-' + str(endyear) + '.png', bbox_inches='tight')
...
#     # Save the image
#     plt.savefig('C:/Users/mieke/Documents/Data_analysis/' + state + '/' + param + '/Plots/' + param +  '_' + level_name + '_' + str(beginyear) + '-' + str(endyear) + '.png', bbox_inches='tight')
#     plt.show()
