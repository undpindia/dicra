...
statebase = '/nfsdata/lai/uttarakhand'
scriptbase= statebase + '/download'
basepath=  '/nfsdata/base_shapefiles'
tifspath = scriptbase + '/GEE_LAItifs'
...
collection_subset = ee.ImageCollection("MODIS/061/MOD15A2H") \
    .sort('IMAGE_DATE').select('Lai_500m') \
...
image = collection_subset.first().select('Lai_500m')  # Pick the first image from the 'list' and select the layer of interest
...
#    os.mkdir(tifspath)
#else:
#    print('GEE_LAItifs directory exists')

#out = os.path.join('GEE_LAItifs') # Set path to where we want to save the GeoTIF
...
geemap.ee_export_image_collection(collection_subset, out_dir = out, scale=image.select('Lai_500m').projection().nominalScale(), region=roi, file_per_band=True, crs='EPSG:4326')
