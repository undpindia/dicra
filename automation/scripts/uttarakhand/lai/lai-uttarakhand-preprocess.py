...
boundryfile = '/nfsdata/uk_district_boundary.geojson'
tifspath = statebase + '/download/GEE_LAItifs'
...
#arr = os.listdir('GEE_LAItifs_scaled') #Don't hardcode the path string in multiple lines, use a variable
...
for i in arr:
   ...
   # ndv = 1
    # apply scale factor
    data = arr_q*(0.1)			#RECALCULATION FORMULA WILL DIFFER FOR EACH DATASET
...
if cogfname not in os.listdir(scriptbase):
    ...
    print('cog_lai directory exists')


for i in arr:
    #cmd="gdal_translate clipped/"+str(i)+" cog_lai/"+str(i)+" -co COMPRESS=LZW -co TILED=YES"
...
#cmd="rm -r GEE_LAItifs_scaled clipped projected recalculated"
#print(cmd)
#try:
#    subp.check_call(str(cmd), shell=True)
#except:
