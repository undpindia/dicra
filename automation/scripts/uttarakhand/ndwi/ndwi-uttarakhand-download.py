...
statebase = '/nfsdata/ndwi/uttarakhand'
scriptbase= statebase + '/download'
basepath=  '/nfsdata/base_shapefiles'
tifspath = scriptbase + '/GEE_NDWItifs'
...
f=  open(basepath+'/uk_state_boundary.geojson')
data =json.load(f)

data = data['features'][0]['geometry']['coordinates'][0]
roi = ee.Geometry.Polygon(data)
collection_subset = ee.ImageCollection("MODIS/MOD09GA_006_NDWI") \
    .sort('IMAGE_DATE').select('NDWI') \
...
image = collection_subset.first().select('NDWI')  # Pick the first image from the 'list' and select the layer of interest
...
#    os.mkdir(tifspath)
#else:
#    print('GEE_NDWItifs directory exists')

#out = os.path.join('GEE_NDWItifs') # Set path to where we want to save the GeoTIF
...
geemap.ee_export_image_collection(collection_subset, out_dir = out, scale=image.select('NDWI').projection().nominalScale(), region=roi, file_per_band=True, crs='EPSG:4326')

# image.select('NDWI').projection().nominalScale() sets the scale equal to the resolution of the images within the collection
