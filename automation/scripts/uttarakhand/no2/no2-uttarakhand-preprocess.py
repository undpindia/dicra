...
#arr = os.listdir('GEE_NO2tifs_scaled') #Don't hardcode the path string in multiple lines, use a variable
...
if cogfname not in os.listdir(scriptbase):
    ...
    print('cog_no2 directory exists')


for i in arr:
    #cmd="gdal_translate clipped/"+str(i)+" cog_no2/"+str(i)+" -co COMPRESS=LZW -co TILED=YES"
...
#cmd="rm -r GEE_NO2tifs_scaled clipped projected recalculated"
#print(cmd)
#try:
#    subp.check_call(str(cmd), shell=True)
#except:
