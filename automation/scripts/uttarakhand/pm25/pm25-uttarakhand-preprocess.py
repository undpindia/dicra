...
#arr = os.listdir('GEE_PM25tifs_scaled') #Don't hardcode the path string in multiple lines, use a variable
...
if cogfname not in os.listdir(scriptbase):
    ...
    print('cog_pm25 directory exists')


for i in arr:
    #cmd="gdal_translate clipped/"+str(i)+" cog_pm25/"+str(i)+" -co COMPRESS=LZW -co TILED=YES"
...
#cmd="rm -r GEE_PM25tifs_scaled clipped projected recalculated"
#print(cmd)
#try:
#    subp.check_call(str(cmd), shell=True)
#except:
