...
jest.mock("react-cool-onclickoutside", () => {
    return {
        __esModule: true,
        default: (callback, options) => { return callback() }
    }
})
jest.mock("use-places-autocomplete", () => {
    return {
        ...
        default: (options, data) => {
             return {
                ...
                suggestions: {
                    loading: false, status: 'OK', data: [{
                        id: 1,
                        structured_formatting: { main_text: 'Welcome', secondary_text: 'Unit Test' },
                    }, {
                        id: 2,
                        structured_formatting: { main_text: 'Hi', secondary_text: 'Hello' },
                    }]
                },
                value: ""
            }
        },
        getGeocode: (data) => { return new Promise((resolve, reject) => { resolve([{}, {}]) }) },
        getLatLng: (data) => { return new Promise((resolve, reject) => { resolve({ lat: 6.7878, lng: 8.8998 }) }) },
    ...
    "Layers": [
        {
            ...
            "citation": " Didan, K. (2015). MOD13A1 MODIS/Terra Vegetation Indices 16-Day L3 Global 500m SIN Grid V006 [Data set]. NASA EOSDIS Land Processes DAAC. Accessed 2022-04-12 from https://doi.org/10.5067/MODIS/MOD13A1.006",
            "short_description": "Normalized difference vegetation index",
            "last_updated": "2022-05-08T00:00:00",
            "long_description": " NDVI quantifies vegetation by measuring the difference between near-infrared (which vegetation strongly reflects) and red light (which vegetation absorbs)",
            "standards": " All data distributed by the LP DAAC contain no restrictions on the data reus",
            "raster_status": true,
            "source": "GLAM NDVIDB",
        ...
        },
        {
            ...
            "update_frequnecy": 0,
            "xaxislabel": null,
            "citation": "Microestimates of wealth for all low- and middle-income countries Guanghua Chi, Han Fang, Sourav Chatterjee, Joshua E. Blumenstock Proceedings of the National Academy of Sciences Jan 2022, 119 (3) e2113658119; DOI: 10.1073/pnas.2113658119",
            "short_description": "Relative Wealth Index",
            "last_updated": "2021-01-01T00:00:00",
            "long_description": "The Relative Wealth Index predicts the relative standard of living within countries using privacy protecting connectivity data, satellite imagery, and other novel data sources.",
            "standards": "Creative Commons Attribution-Non Commercial 4.0 International (CC BY-NC 4.0)",
            "raster_status": true,
...
describe("Test Map Component", () => {
    ...
    it("render Map component with status OK to load suggestions list", () => {
        ...
        let component = shallow(
            <Map1 LayerDescription={layerDescription} setval={storeData.setval} setplace={storeData.setplace} Layers={storeData.Layers}
                DownloadLayer={storeData.DownloadLayer} DownloadLayerDate={storeData.DownloadLayerDate} DownloadLayerType={storeData.DownloadLayerType}
                DownloadLayerRegion={storeData.DownloadLayerRegion} CurrentLayer={storeData.CurrentLayer} RasterLoader={storeData.RasterLoader}
                VectorLoader={storeData.VectorLoader} CurrentVector={storeData.CurrentVector} CurrentRegion={storeData.CurrentRegion} MapKey={storeData.MapKey}
                setColor={storeData.setColor} DownloadLayerDesc={storeData.DownloadLayerDesc} ShowDrawer={storeData.ShowDrawer} RasterOpacity={storeData.RasterOpacity}
                DownloadFile={storeData.DownloadFile} CurrentLayerType={storeData.CurrentLayerType} Hoverlatlon={storeData.Hoverlatlon}
    ...
    it('should generate outside click for suggestions and closes suggestions', () => {
        ...
        const component = enzyme.shallow(<SearchPlace searchArea={getcustomlocation} />);
    ...
    it('should cover search icon', () => {
        ...
        component.mapInstance = { leafletElement: '' };
        console.log(component.find('.search-icon').at(0).simulate('click'), 'in div', component.state('activeSearch'))
    ...
    it('should create custom layer', async () => {
        ...
        await mapInstance.Customlayer({
            target: { value: 'Raster' }, layer: {
                _latlngs: [[{ lng: 0.988, lat: 0.889 }]], on: (val) => {
    ...
    it('should cover connect store', () => {
        ...
        component.find('#abc');
        render(<Provider store={store}><Map LayerDescription={layerDescription} setval={storeData.setval} setplace={storeData.setplace} Layers={storeData.Layers}
            DownloadLayer={storeData.DownloadLayer} DownloadLayerDate={storeData.DownloadLayerDate} DownloadLayerType={storeData.DownloadLayerType}
            DownloadLayerRegion={storeData.DownloadLayerRegion} CurrentLayer={storeData.CurrentLayer} RasterLoader={storeData.RasterLoader}
            VectorLoader={storeData.VectorLoader} CurrentVector={storeData.CurrentVector} CurrentRegion={storeData.CurrentRegion} MapKey={storeData.MapKey}
            setColor={storeData.setColor} DownloadLayerDesc={storeData.DownloadLayerDesc} ShowDrawer={storeData.ShowDrawer} RasterOpacity={storeData.RasterOpacity}
            DownloadFile={storeData.DownloadFile} CurrentLayerType={storeData.CurrentLayerType} Hoverlatlon={storeData.Hoverlatlon}
            setvalue={storeData.setvalue} /></Provider>)
    })
    it('should create ura', () => {
        let mapInstance = new Map1(storeData);
        mapInstance.onEachrua({ properties: { Dist_Name: 'Hydrabad' } }, { bindPopup: (ruaname) => { } })
    ...
    it('it should open drawer based on currentlayer FIREEV', () => {
        ...
        mapInstance.child={current:{showDrawer:()=>{},getWeathertrend:(duration)=>{}}}
        mapInstance.openDrawer({ sourceTarget: { feature: { properties: { Dist_Name: 'Hydrabad', zonalstat: { min: 0.2, max: 0.8 } }, geometry: '' } } })
