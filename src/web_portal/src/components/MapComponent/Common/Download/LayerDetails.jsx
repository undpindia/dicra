...
const mapStateToProps = (props) => {
  return {
    ...
    SelecterCategory: props.SelecterCategory,
    SelecterCategoryId: props.SelecterCategoryId,
  };
};
...
class LayerDetails extends Component {
  ...
  changeLayer(e) {
    this.props.setdownloadlayerdesc(
      this.state.layerdata[this.state.selectCategory_id][e.target.selectedIndex]
    );
    ...
    this.setState(
      {
        ...
        layerDesc:
          this.state.layerdata[this.state.selectCategory_id][
  ...
  async getavailableFiles(id) {
    try {
      const res = await getdownloadfiles(id);
      this.setState(
        {
  ...
  async getLayerdetails() {
    try {
      const layers = await getlayers(this.props.LayerDescription.region_id);
      let result;
          ...
          <div className="downloads-content">
              ...
              <input
                ...
                onClick={() => {
                  if (this.state.layerdata[1][0].id) {
                    ...
                    this.props.setdownloadlayerdesc(this.state.layerdata[1][0]);
                    this.getavailableFiles(this.state.layerdata[1][0].id);
                    this.props.setdownloadlayer(
                      this.state.layerdata[1][0].layer_name
                    );
                  } else {
                    this.setState({
                      selecterCategory: 'SOCIO-ECONOMIC',
              ...
              <input
                ...
                onClick={() => {
                  ...
                  if (this.state.layerdata[2][0].id) {
                    ...
                    this.props.setdownloadlayerdesc(this.state.layerdata[2][0]);
                    this.getavailableFiles(this.state.layerdata[2][0].id);
                    this.props.setdownloadlayer(
                      this.state.layerdata[2][0].layer_name
                    );
                  } else {
                    this.setState({
                      selecterCategory: 'ENVIRONMENTAL',
              ...
              <input
                ...
                onClick={() => {
                  if (this.state.layerdata[3][0].id) {
                    ...
                    this.props.setdownloadlayerdesc(this.state.layerdata[3][0]);
                    this.getavailableFiles(this.state.layerdata[3][0].id);
                    this.props.setdownloadlayer(
                      this.state.layerdata[3][0].layer_name
                    );
                  } else {
                    this.setState({
              ...
              <input
                ...
                onClick={() => {
                  if (this.state.layerdata[4][0].id) {
                    ...
                    this.props.setdownloadlayerdesc(this.state.layerdata[4][0]);
                    this.getavailableFiles(this.state.layerdata[4][0].id);
                    this.props.setdownloadlayer(
                      this.state.layerdata[4][0].layer_name
                    );
                  } else {
                    this.setState({
                      selecterCategory: 'POSITIVE DEVIANCE & NEGATIVE DEVIANCE',
            ...
            <div style={{ padding: '10px' }}>
              ...
              >
                ...
                undefined
                  ? console.log()
                  : this.state.layerdata[this.state.selectCategory_id].map(
                      (item, index) => (
                        <option key={item.id} disabled={!item.isavailable}>
                          {item.display_name}
                        </option>
                      )
                    )}
